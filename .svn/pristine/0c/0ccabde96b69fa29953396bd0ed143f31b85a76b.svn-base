...
//        return date("Y-m-d", strtotime("2019-01-31 +1 month"));
        $this->load->model('vendas_models');
        ...
        $dia_parametro = $meta_venda_apartir_dia; // geralmente é 26 ou 01
        $dia = date('d');
        ...
        if ($dia_parametro == 01) {
            ...
            if ($dia > $dia_parametro) {
                ...
                $data_init = date('Ymd', strtotime('+1 month', strtotime($data_init)));
                $data_final = date('Ymd', strtotime('+1 month', strtotime($data_final)));
                $ultimo_dia = cal_days_in_month(CAL_GREGORIAN, date('m'), date('Y')); // 31
                
//                return $ultimo_dia;
                if ($ultimo_dia == 31 && ($dia >= 26 && $dia <= 31)) {
                    $data_meta = substr($data_final, 0, 6);
                }
                if ($data_base != $data_meta && $dia > $ultimo_dia) {
                    $data_meta = $data_base;
                }
            } else {
                $data_meta = $data_base;
        ...
        return array('$data_meta' => $data_meta, '$data_init' => $data_init, '$data_final' => $data_final, '$vendedor_ext' => $vendedor_ext, '$dia' => $dia, '$dia_parametro' => $dia_parametro, '$dia > $dia_parametro' => $dia > $dia_parametro );
...
//        echo json_encode(array('$data_init' => $data_init, '$data_final' => $data_final, '$data_meta' => $data_meta, '$dia' => $dia, '$dia_parametro' => $dia_parametro, '$dia > $dia_parametro' => $dia > $dia_parametro));
//        exit();
        ...
        $mes = substr($data_meta, 4, 2).'/'.substr($data_meta, 0, 4);
