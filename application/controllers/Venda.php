...
class Venda extends MY_Controller {
    ...
    public function listaClientesAjax() {
        ...
        $retorno = $this->clientes_models->listaClientesAjax($this->input->get('q'), $vendedor, $onlineVisClientes, $usuarioVisualizarVendas, $usuarioVenderClienteBloq, $parametroVisualizarVendas);
    ...
    public function listaProdutoAjax() {
        ...
        $parametros = array(
            ...
            'TABELA_PRECO_MINIMO_PEDIDO' => $this->session->userdata('TABELA_PRECO_MINIMO_PEDIDO')
        );
    ...
    public function addProdutoTabela($alterar = 'N', $codigo_produto = NULL, $descricao = NULL, $valor_unit = NULL, $quantidade = NULL, $desconto = NULL, $codigo_cliente = NULL, $estado = NULL, $optante_simples = NULL, $aliquota_ipi = NULL, $sequencia = NULL, $dif_st = NULL, $data_fechamento = NULL, $custo_bruto = NULL, $acrescimo = NULL, $grupo = NULL, $observacao = NULL) {
        $this->load->model('produtos_models');
    ...
    public function inserirPedido() {
        ...
        for ($x = 0; $x < count($item); $x++) {
            ...
            $dadosItens = array(
                ...
                'SEQUENCIA' => $item[$x]->sequencia_produto,
                'UNIDADE' => 'UN',
                ...
                'RETIRAR' => 'N',
                'VP' => 'N',
                'OBSERVACAO' => $item[$x]->obs_item_table
            );
...
//        $this->load->view('telas/relatorios/view_vendas', $novas_vendas);
    ...
    public function visualizarObsItemPedido() {
        $this->load->model('vendas_models');
        
        $codigo = $this->input->get('codigo');
        $sequencia = $this->input->get('sequencia');
        
        $numero_pedido = $this->input->get('numero_pedido');
        $incluir_alterar = $this->input->get('tipo_orc');
        ...
        $result = $this->vendas_models->buscarItensPedido($numero_pedido, $codigo, $tableItens, $sequencia);
        
        echo json_encode($result);
    }
    
    public function alterarObsItemPedido() {
        $this->load->model('vendas_models');
        ...
        $result = $this->vendas_models->alterarObsItemPedidoModels($numero_pedido, $codigo, $sequencia, $observacao);
